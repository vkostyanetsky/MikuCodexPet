...
* **`.github/workflows/build-pet.yml`** — a manually dispatched GitHub Actions workflow that rebuilds the atlas and publishes `miku-codex-pet.zip`, an archive rooted at a `miku/` folder so it unpacks straight into `~/.codex/pets/`. Dispatching it from a `v*` tag also attaches the archive to that tag's release.
