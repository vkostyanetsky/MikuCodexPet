...
* **`.github/workflows/release.yml`** — a manually dispatched GitHub Actions workflow that builds the pet from the tag you name and opens a draft release with `miku-codex-pet.zip` attached, an archive rooted at a `miku/` folder so it unpacks straight into `~/.codex/pets/`.
